...
st.markdown(
    ...
    <style>
        ...
        html, body, [class*="css"] { color: #eaf2ff; }
...
def render_landing():
    ...
    st.markdown(f'<div class="event-title">{CONFIG["event_name"].split()[0]} {CONFIG["event_name"].split()[1]}</div>', unsafe_allow_html=True)
...
def render_game():
    ...
    st.markdown(f'<div class="qtitle">Question {q_index + 1} of {len(QUESTIONS)} — {q["title"]}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="qprompt">{q["prompt"]}</div>', unsafe_allow_html=True)
    if q["snippet"]:
        st.markdown(f'<div class="qsnippet">{q["snippet"]}</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.answer_locked:
        ...
        with b2:
            if st.button("⏭️ Skip", use_container_width=True):
                ...
                st.rerun()
        with b3:
            if q.get("hint") and st.button("💡 Hint (-{} pts)".format(CONFIG["hint_penalty"]), use_container_width=True, disabled=st.session_state.show_hint):
    ...
    st.write("")
    if st.button("↺ Switch Player / Restart", use_container_width=False):
        reset_game()
        st.rerun()
...
def render_finish():
    ...
    with mid:
        ...
        c4.metric("Time", f"{mins:02d}:{secs:02d}")
